...
const AssessmentQuiz = ({ questions, onComplete }) => {
  ...
  try {
    const userEmail = user?.email; // تأكد إنك جايب الإيميل من السياق أو state

    const response = await apiInstance.post("/create-payment/", {
      email: userEmail,
      course_id: courseId,
    });
    ...
    if (response.data?.redirect_url) {
      ...
      alert("تم إرسال الطلب ولكن لم يتم استلام رابط الدفع");
    }

  } catch (error) {
    console.error("خطأ أثناء إنشاء رابط الدفع:", error);
    ...
    if (!questions || questions.length === 0) {
     ...
     console.log("زر الدفع اتضغط، course ID:", courseId);
    }
...
// //   if (!userEmail) {
// // //         alert("تعذر الحصول على البريد الإلكتروني.");
// // //         return;
// // //     }
// //
//
//     try {
//         const response = await axios.post("http://127.0.0.1:8000/api/v1/create-payment/", {
//
//             course_id: courseId,
//         });
//
...
const handleSubscribe = async (courseId) => {
  try {
    ...
    let response = await axios.post(
      "https://fatihcenter.com/api/v1/create-payment/",
      {
        course_id: courseId,
        type: "course", // لازم string مش variable
      },
      {
        headers: {
    ...
    // لو التوكن منتهي أو فيه مشكلة auth
    if (error.response && error.response.status === 401) {
      console.log("انتهى التوكن، بنجدد...");
      ...
      if (newToken) {
        ...
        try {
          ...
          if (retryResponse.data.status === "success") {
            const paymentUrl = retryResponse.data.data.url;
            window.location.href = paymentUrl;
            return;
          } else {
            alert("فشل في إنشاء رابط الدفع.");
          ...
          console.error("فشل بعد تجديد التوكن:", retryError);
          alert("حدث خطأ أثناء معالجة الدفع.");
        }
      } else {
